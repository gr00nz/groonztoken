...
contract('GroonzTokenIco', accounts => {
  ...
  const createToken = () => GroonzTokenIco.new(fundsWallet, timeController.currentTimestamp(), minCap, maxCap);

  // REQ001: Basic ERC20 “Groonz Token” with symbol of "GRZ", 
  ...
  it('should have initial supply of 1,000,000 units assigned to funds wallet', async () => {
    ...
    const groonzToken = await createToken();
  ...
  it('should open at startTimestamp', async () => {
    ...
    const groonzToken = await GroonzTokenIco.new(fundsWallet, startTimestamp, minCap, maxCap);

    // should be closed
    //await assertExpectedError(groonzToken.sendTransaction(transaction(buyerOneWallet, oneEth)));

    await timeController.addSeconds(3600);

    // should be open
    await groonzToken.sendTransaction(transaction(buyerOneWallet, oneEth));
  });

  it('should last 4 weeks if the goal is reached and allow token transfers afterwards', async () => {
    const groonzToken = await createToken();

    await groonzToken.sendTransaction(transaction(buyerOneWallet, minCap));
    await timeController.addDays(4 * 7);

    // should be closed
    //await assertExpectedError(groonzToken.sendTransaction(transaction(buyerTwoWallet, oneEth)));

    const totalRaised = await groonzToken.totalRaised();
    assert.equal(totalRaised.toNumber(), minCap, 'Total raised amount mismatch')

    // REQ004 should allow token transfer
    await groonzToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet });
  });
  ...
  it('should last more then 4 weeks if the goal is not reached and allow token transfers after closing', async () => {
    ...
    await groonzToken.sendTransaction(transaction(buyerTwoWallet, minCap - oneEth));

    // should be closed
    //await assertExpectedError(groonzToken.sendTransaction(transaction(buyerThreeWallet, oneEth)));
  ...
  it('should close after the max cap is reached and allow token transfers afterwards', async () => {
    ...
    await groonzToken.sendTransaction(transaction(buyerTwoWallet, maxCap));
  ...
  it('should transfer raised funds to fundsWallet after each payment', async () => {
    ...
    assert.equal(ethBalance(groonzToken.address), 0, 'Contract balance mismatch');
    assert.equal(ethBalance(fundsWallet), expectedBalanceGrowth(oneEth), 'Funds wallet balance mismatch');

    await groonzToken.sendTransaction(transaction(buyerTwoWallet, oneEth));
  ...
  it('should not allow token transfers before ICO', async () => {
    ...
    //await assertExpectedError(groonzToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet }));
  });
  ...
  it('should be sold with +50% bonus during the first week', async () => {
    ...
    const buyerOneBalance = await groonzToken.balanceOf(buyerOneWallet);
